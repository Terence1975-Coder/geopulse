...
export default function LiveSignalsWorkspace({ signals, onOpenSignal }: Props) {
  ...
  const sourceTypes = ["all", ...Array.from(new Set(signals.map((s) => s.source_type || "unknown")))];
  const lifecycles = ["all", ...Array.from(new Set(signals.map((s) => s.lifecycle)))];
  ...
  const filtered = useMemo(() => {
    ...
    const sorted = [...subset].sort((a, b) => {
      if (sortBy === "freshest") return a.freshness_minutes - b.freshness_minutes;
    ...
    <div className="space-y-6">
      <section className="rounded-[32px] border border-white/10 bg-gradient-to-br from-slate-900/90 via-slate-950/90 to-indigo-950/30 p-6 shadow-2xl">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-7">
            ...
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as "freshest" | "strongest" | "confidence")}
              className="rounded-2xl border border-white/10 bg-slate-950/60 px-4 py-3 text-sm text-white"
            >
      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <MetricCard
          ...
          value={`${Math.round(
            (filtered.reduce((acc, item) => acc + item.confidence_score, 0) /
