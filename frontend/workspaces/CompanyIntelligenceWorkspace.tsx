...
export default function CompanyIntelligenceWorkspace({
  ...
  const handleSaveCalibration = async () => {
    try {
      ...
      // local-only save for now, until a real backend endpoint is added
      await new Promise((resolve) => setTimeout(resolve, 700));

      setSaveStatus("Saved locally. Backend persistence not wired yet.");
    } catch {
      ...
      <div className="space-y-6">
        <div className="rounded-3xl border border-cyan-400/20 bg-cyan-500/10 p-6">
          ...
          <p className="mt-2 text-sm leading-7 text-slate-200">
            Live-compatible scaffold for external company enrichment and profile
            population.
          </p>
          ...
          <div className="mt-6 flex gap-3">
            ...
            <button
              type="button"
              className="rounded-2xl border border-cyan-400/20 bg-cyan-400/10 px-6 py-3 text-sm font-medium text-cyan-100"
            >
              Search
            </button>
          </div>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <div className="rounded-3xl border border-white/10 bg-black/20 p-5">
              ...
              <div className="mt-4 text-sm text-white/70">{saveStatus}</div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
          ...
          <p className="mt-5 text-base leading-8 text-slate-300">
            Company context is used to calibrate risk interpretation, personalise
            opportunity scoring, improve advisor recommendations, and refine
            scenario relevance.
          </p>
        </div>
      </div>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm leading-7 text-slate-300">
                Edit company context, then save it into GeoPulse calibration
                memory.
              </p>
            </div>
            ...
            <button
              ...
              className="rounded-2xl border border-cyan-400/20 bg-cyan-400/10 px-5 py-3 text-sm font-medium text-cyan-100 transition hover:bg-cyan-400/20"
            >
              Save Calibration
            </button>
          </div>
