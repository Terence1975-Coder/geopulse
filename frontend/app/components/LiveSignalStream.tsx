  ...
  return (
    <div className="space-y-3">
      {signals.map((signal) => {
        ...
        const motionConfig = urgencyMotion(signal.urgency, reduced);
        ...
        return (
          <motion.article
            ...
            animate={{
              opacity: signal.urgency === "background" ? 0.78 : 1,
              x: 0,
          ...
          >
            ...
            <div
              className={`mt-4 rounded-2xl border border-white/6 bg-gradient-to-r ${fresh.surface} px-3 py-3`}
            >
              <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
                <div className="text-slate-300">
                  Freshness:{" "}
                  <span className={`${fresh.tone} font-medium`}>
                    {fresh.label}
                  </span>
                </div>
              ...
              <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/6">
                <motion.div
                  ...
                  initial={{ width: "100%" }}
                  animate={{
                    width: `${Math.max(12, 100 - Math.min(old, 100))}%`,
                  }}
