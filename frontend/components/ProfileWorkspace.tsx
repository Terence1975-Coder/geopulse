...
export default function ProfileWorkspace() {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: crypto.randomUUID(),
          role: "assistant",
          content: data?.outputs?.profile || "No profile response returned.",
          timestamp: new Date().toISOString(),
        },
      ]);
    } catch {
      setMessages((prev) => [
        ...prev,
        {
          id: crypto.randomUUID(),
          role: "assistant",
          content: "Profile agent failed to respond.",
          timestamp: new Date().toISOString(),
        },
      ]);
    } finally {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-slate-900 p-4 text-white">
      <h2 className="mb-4 text-xl font-semibold">Company Profile Agent</h2>
        ...
        {messages.map((message) => (
          <div
            ...
            className={`rounded-xl p-3 ${
              message.role === "user" ? "bg-fuchsia-900/40" : "bg-slate-800"
            }`}
          >
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder="Ask the Company Profile Agent..."
        />
        <button
          onClick={handleSend}
          className="rounded-xl bg-fuchsia-600 px-4 py-3 font-medium text-white"
        >
          Send
        </button>
      </div>
